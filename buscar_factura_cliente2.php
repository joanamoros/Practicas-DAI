<?php
    include("auth.php");
    include("conexionPDO.php");

    $facturas = array();
    $id_cliente = 0;
    $nombreCliente = '';

    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["buscar_por_cliente"])) {
        $id_cliente = intval($_POST["id_cliente"]);

        $stmtCliente = $conexion->prepare("SELECT Nombre, Apellido1, Apellido2 FROM Clientes WHERE Id_Cliente = ?");
        $stmtCliente->execute([$id_cliente]);
        $cliente = $stmtCliente->fetch();
        if ($cliente) {
            $nombreCliente = $cliente['Nombre'] . ' ' . $cliente['Apellido1'] . ' ' . $cliente['Apellido2'];
        }

        // Las facturas se relacionan con el cliente A TRAVÉS de la moto:
        // Facturas.Matricula -> Motocicletas.Matricula -> Motocicletas.Id_Cliente
        $sql = "SELECT f.* FROM Facturas f
                JOIN Motocicletas m ON f.Matricula = m.Matricula
                WHERE m.Id_Cliente = ?
                ORDER BY f.Fecha_Emision DESC";
        $consulta = $conexion->prepare($sql);
        $consulta->execute([$id_cliente]);
        $facturas = $consulta->fetchAll();
    }
?>
<!DOCTYPE html>
<html lang="es">
<head>
    <link rel="stylesheet" href="estilo.css">
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
    <title>Resultados de búsqueda de facturas</title>
</head>
<body>
    <?php include("topbar.php"); ?>
    <div class="container">
        <h1 class="text-center my-4"><b>Resultados de búsqueda de facturas</b></h1>
        <h2>Listado de Facturas de <?php echo htmlspecialchars($nombreCliente, ENT_QUOTES, 'UTF-8'); ?></h2>
        <?php if ($facturas) { ?>
            <table class="table table-bordered">
                <thead><tr><th>Número de factura</th><th>Fecha de emisión</th><th>Fecha de pago</th><th>Total</th></tr></thead>
                <tbody>
                <?php foreach ($facturas as $factura) { ?>
                    <tr>
                        <td><a href="listar_facturas2.php?id=<?php echo urlencode($factura['Numero_Factura']); ?>"><?php echo htmlspecialchars($factura['Numero_Factura'], ENT_QUOTES, 'UTF-8'); ?></a></td>
                        <td><?php echo htmlspecialchars($factura['Fecha_Emision'], ENT_QUOTES, 'UTF-8'); ?></td>
                        <td><?php echo htmlspecialchars($factura['Fecha_Pago'], ENT_QUOTES, 'UTF-8'); ?></td>
                        <td><?php echo number_format((float)$factura['Total'], 2); ?> €</td>
                    </tr>
                <?php } ?>
                </tbody>
            </table>
        <?php } else { ?>
            <p>No se encontraron facturas para el cliente especificado.</p>
        <?php } ?>
    </div>
</body>
</html>
